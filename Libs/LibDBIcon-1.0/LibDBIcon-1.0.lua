...
local function getAnchors(frame)
	...
	if not x or not y or issecretvalue(x) or issecretvalue(y) then return "CENTER" end
...
do
	...
	function updatePosition(button, position)
		...
		if not mapW or not mapH or issecretvalue(mapW) or issecretvalue(mapH) then return end
		local w = (mapW / 2) + lib.radius
		local h = (mapH / 2) + lib.radius
		if quadTable[q] then
			x, y = x*w, y*h
		else
...
do
	...
	local function onUpdate(self)
		...
		if not mx or issecretvalue(mx) or not my or issecretvalue(my) or not scale or issecretvalue(scale) then return end
